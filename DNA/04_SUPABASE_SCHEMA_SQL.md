...
- `projects.agent_project_id` (= `projectId` del JSON Sync) e' un codice alfanumerico casuale `prj-xxxxx` per TUTTI i progetti (`generateProjectId`). I progetti storici avevano slug derivati dal nome: consolidati al nuovo formato con `20260624_01_consolidate_project_ids.sql`. Cambiare lo slug NON tocca i dati (legati a `projects.id` UUID), ma invalida i vecchi `.agent/app-control.json`: dopo il consolidamento ogni progetto va ri-sincronizzato copiando il JSON aggiornato dal tab Sync.
