...
- `projects.agent_project_id` (= `projectId` del JSON di collegamento) e' un codice alfanumerico casuale `prj-xxxxxxxxxx` (10 char) per i nuovi progetti (`generateProjectId`); i vecchi `prj-xxxxx` (5 char) restano validi. I progetti storici avevano slug derivati dal nome: consolidati al nuovo formato con `20260624_01_consolidate_project_ids.sql`. Cambiare lo slug NON tocca i dati (legati a `projects.id` UUID), ma invalida i vecchi `.agent/app-control.json`: dopo il consolidamento ogni progetto va ri-sincronizzato copiando il JSON aggiornato dalla sezione in fondo al tab `Dati progetto`.
- `LINK_DEPLOY` (link pubblico/user) e `LINK_DEPLOY ADMIN` (link admin reale) sono due variabili distinte in `project_env_variables`, entrambe valorizzate dall'Agent con i link reali del progetto. Il link admin varia da progetto a progetto: niente default con suffisso fisso, l'Agent lo ricava dal codice/config reali e lo mantiene aggiornato.
