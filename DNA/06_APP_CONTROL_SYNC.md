...
L'agent lo legge da solo a inizio sessione e si collega ad App Control con header `x-app-control-project-id` + `x-app-control-agent-key` + anon key. Il JSON di collegamento `.agent/app-control.json` (e la chiave agent) si **genera in automatico** alla creazione di un nuovo progetto (`createEmptyProject` -> `createProjectRecord`) e si copia dalla sezione in fondo al tab `Dati progetto`. Nota: il progetto App Control stesso ha `agentKey` vuoto perche precede questa funzione.

### Fonte unica del prompt di Sync (vincolante)
Il testo del prompt di sync e definito da **`defaultSyncPrompt`** in `projectPageConstants.ts` e viene salvato nel DB (`project_agent_keys.sync_prompt`) alla creazione di ogni progetto. NON e piu mostrato in UI: il tab `Sync` e stato rimosso (il flusso di collegamento e ora automatizzato dai prompt operativi `AVVIO SESSIONE` / `BOOTSTRAP` + `CLAUDE.MD`). Del pannello Sync resta in UI solo il **JSON di collegamento `.agent/app-control.json`**, spostato in fondo al tab `Dati progetto` (componente `ProjectAgentConfig`). Le costanti legacy dei prompt e `resolveSyncPrompt` sono state rimosse (erano usate solo dal vecchio pannello). Il prompt **BOOTSTRAP** e il prompt **CLAUDE.MD** (tabella `prompts`) restano la fonte canonica del flusso di sync: non devono divergere.
